...
#include "paam_server_interfaces/msg/gpu_register.hpp"
#include "paam_server_interfaces/msg/admissions_response.hpp"
...
class paamServer : public rclcpp::Node
{

public:
  rclcpp::TimerBase::SharedPtr timer_;
  rclcpp::Publisher<paam_server_interfaces::msg::GPURegister>::SharedPtr publisher_;
  rclcpp::Publisher<paam_server_interfaces::msg::GPURequest>::SharedPtr remote_publisher_;
  rclcpp::Publisher<paam_server_interfaces::msg::AdmissionsResponse>::SharedPtr admissions_publisher_;
  rclcpp::Subscription<paam_server_interfaces::msg::GPURequest>::SharedPtr subscription_;
  rclcpp::Subscription<paam_server_interfaces::msg::GPURegister>::SharedPtr register_sub_;
  rclcpp::Subscription<sensor_msgs::msg::Image>::SharedPtr image_sub_;
  rclcpp::Subscription<paam_server_interfaces::msg::Admissions>::SharedPtr admissions_sub_;
  paamServer() : Node("paam_server"), count_(0)
  {
    // RCLCPP_INFO(this->get_logger(), "Making Publishers");
    publisher_ = this->create_publisher<paam_server_interfaces::msg::GPURegister>("handshake_topic", 1000);
    remote_publisher_ = this->create_publisher<paam_server_interfaces::msg::GPURequest>("remote_response_topic", 1000);
    admissions_publisher_ = this->create_publisher<paam_server_interfaces::msg::AdmissionsResponse>("admissions_response_topic", 1000);
    // std::this_thread::sleep_for(7000ms);
    subscription_ = this->create_subscription<paam_server_interfaces::msg::GPURequest>(
        "request_topic", 1024, std::bind(&paamServer::topic_callback, this, std::placeholders::_1));
    register_sub_ = this->create_subscription<paam_server_interfaces::msg::GPURegister>(
        "registration_topic", 1024, std::bind(&paamServer::register_client_callback, this, std::placeholders::_1));
    // admissions_sub_ = this->create_subscription<paam_server_interfaces::msg::Admissions>("admissions_topic", 10, std::bind(&paamServer::admissions_callback, this, std::placeholders::_1));
    ...
    // std::thread{ std::bind(&paamServer::runtime_monitor, this) }.detach();
    std::thread{&paamServer::runtime_monitor, this}.detach();
    this->init_tpu();
    this->use_tpu();
    this->attach_logging_shm();
    // std::thread rm_t(paamServer::runtime_monitor).detach();
  }
...
private:
  ...
  {
  public:
    bool operator()(const paam_server_interfaces::msg::GPURequest::SharedPtr Request1,
                    const paam_server_interfaces::msg::GPURequest::SharedPtr Request2)
    {
  ...
  std::priority_queue<std::shared_ptr<paam_server_interfaces::msg::GPURequest>,
                      std::vector<std::shared_ptr<paam_server_interfaces::msg::GPURequest>>, compare_priority>
      stream_queues[8];
  ...
  void clean_working_queues(std::string callback_name, callback_key_struct *delete_struct)
  {
    ...
    std::vector<std::shared_ptr<paam_server_interfaces::msg::GPURequest>> queue_list; // temporary list
    int i = 0;
  ...
  {
    std::vector<std::shared_ptr<paam_server_interfaces::msg::GPURequest>> queue_list;
  ...
  void init_worker_threads(void)
  {
    ...
    {
      // worker_threads[i] = std::thread{&paamServer::gpu_worker_thread, this, i}; //.detach();
    ...
    // std::thread{&paamServer::opencv_worker, this}.detach();
    // std::thread{&paamServer::tpu_worker, this}.detach();
  }
  ...
  void topic_callback(paam_server_interfaces::msg::GPURequest::SharedPtr request)
  {
#ifdef OVERHEAD_DEBUG
    ...
    //  std::thread{ std::bind(&paamServer::enqueue, this, request) }.detach();
    //  auto msg = std::make_shared<paam_server_interfaces::msg::GPURequest>();
    //*msg = *request;
    //  this->enqueue(msg);
  ...
  void remote_register_logic(paam_server_interfaces::msg::GPURegister::SharedPtr request)
  {
    ...
    // callback_key_map_mutex.unlock();
    paam_server_interfaces::msg::GPURegister response = *request; // makes a response and copies data
  ...
  void register_logic(paam_server_interfaces::msg::GPURegister::SharedPtr request)
  {
    std::string callback_uuid = this->toHexString(request->uuid);
  ...
  std::vector<std::vector<callback_row>> pending_chain_list;
  void register_client_callback(paam_server_interfaces::msg::GPURegister::SharedPtr request)
  {
    // if( request->admissions_control){
    //   if(request->callback_row.chain_id > pending_chain_list.size()){
    //     pending_chain_list.resize(request->callback_row.chain_id);
    //   }
    //   pending_chain_list[request->callback_row.chain_id].push_back(make_callback_rows(request->callback_row));
    // }   
    if (request->remote)
    {
  ...
  inline void enqueue(paam_server_interfaces::msg::GPURequest::SharedPtr shared_request)
  {
    //  Determine Bucket
    bool notify = false;
...
int main(int argc, char *argv[])
{
  ...
  auto gpu_server = std::make_shared<paamServer>();
